...
function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full px-11 py-4 flex justify-between items-center bg-white/3 backdrop-blur-[5px]">
      ...
      <div className="links hidden md:flex gap-10">
        {navItems.map((item, index) => (
          <Link
            key={index}
            to={item.to}
            className={`relative overflow-hidden h-7 cursor-pointer group ${
              index === 3 && "ml-32"
            }`}
          >
            {/* Top Text */}
            <div className="transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:-translate-y-full text-lg capitalize">
              {item.label}
            </div>

            {/* Bottom Text */}
            <div className="absolute left-0 top-full transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:-translate-y-full text-lg capitalize">
              {item.label}
            </div>
          </Link>
        ))}
      ...
      {/* Mobile Slide Menu */}
      <div
        className={`fixed top-0 left-0 h-screen w-2/3 z-50 bg-white backdrop-blur-lg shadow-xl transform transition-transform duration-500 ease-in-out md:hidden
